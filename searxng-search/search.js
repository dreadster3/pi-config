...
if (!query) {
	...
	console.log("  --content                 Fetch readable page content as markdown");
	console.log("  --time-range <range>      Filter: day | week | month | year");
	console.log("  --categories <cats>       Comma-separated: general, news, images, science, it");
	console.log("  --language <lang>         Language code, e.g. en, de, fr (default: en)");
	console.log("  --url <url>               SearXNG base URL (default: https://searxng)");
	console.log("");
	console.log("Examples:");
	console.log('  search.js "javascript async await"');
	console.log('  search.js "rust programming" -n 10');
	console.log('  search.js "climate change news" --categories news --time-range week');
...
async function fetchSearxngResults(query, numResults) {
	...
	try {
		...
		if (e.name === "TimeoutError") {
			throw new Error(`SearXNG request timed out after 15s. Is the instance running at ${searxngUrl}?`);
		}
		throw new Error(`Could not connect to SearXNG at ${searxngUrl}: ${e.message}`);
	}
	...
	if (!data.results || !Array.isArray(data.results)) {
		throw new Error("Unexpected SearXNG response format — missing 'results' array.");
	}
...
async function fetchPageContent(url) {
	try {
		...
		const fallbackDoc = new JSDOM(html, { url, virtualConsole: makeSilentVirtualConsole() });
